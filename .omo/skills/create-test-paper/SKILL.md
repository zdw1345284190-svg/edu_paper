...
已有某份试卷，需要出一份**相同知识点但全部更换数字/表述**的重测版，检测学生是否真正掌握。

### 步骤

1. 分析原卷每道题对应的**知识点**（如余角、平行线同位角、三角形内角和等）
2. 逐题替换数值和表述，保持知识点不变
3. 更新答案密钥（SKEY/FKEY/FALT）
4. 使用**唯一 localStorage key**（SK），与原始试卷不冲突
5. 在 `index.html` 中添加链接，更新计数
...
|------|--------|------|------|
| Q1 | 余角 | 35°→55° | 67°→23° |
| Q2 | 平行线同位角 | 50°→50° | 41°→41° |
| ... | ... | ... | ... |

> **经验**：填完此表再写代码，避免前后矛盾或知识点遗漏。

### 答案密钥更新规范

```javascript
// 原卷（geo_260724）
var SK='geo_260724',SKEY={q1:'C',q2:'A',q3:'D',q4:'D',q5:'B',q6:'C'},FKEY={...};

// 重测卷 — SK 必须唯一，密钥全部更新
var SK='geo_260731_v3',SKEY={q1:'A',q2:'A',q3:'C',q4:'C',q5:'B',q6:'C'},FKEY={...};
```
...
| 原数值 | 替换为 | 说明 |
|--------|--------|------|
| 35°（余角→55°） | 67°（余角→23°） | 不要用太接近的数字（如36°→54°容易被识破） |
| 3倍（补角方程） | 2倍 / 4倍 | 改变倍数关系，避免算出来还是45° |
| ∠A=50°,∠B=70°（外角） | ∠A=65°,∠B=55° | 内外角值同时换 |
| 时钟 3:00→3:15 | 2:00→2:20 / 4:00→4:25 | 变换时间段和起始时间 |
| 平行判定"同位角相等" | "内错角相等" / "同旁内角互补" | 用不同判定定理 |
| 三角方程关系 | ∠A=2∠B=3∠C → ∠A:∠B:∠C=2:3:5 → ∠B=2∠A,∠C=3∠A | 改关系模式 |

## 新沉淀：打印时答案隐藏（`@media print` 安全）

### 问题

```css
/* ❌ 危险：打印时显示所有答案，未点击"显示答案"也泄露 */
@media print {
  .answer-reveal { display: block !important; background: #f9f9f9; }
}
```

Ctrl+P 时，所有 `.answer-reveal` 被强制显示，学生还没点「显示答案」就知道答案了。

### 修复

```css
/* ✅ 安全：只有已通过密码显示答案的元素才在打印中可见 */
@media print {
  .answer-reveal.show { display: block !important; background: #f9f9f9; }
}
```

### 排查清单

- [ ] `@media print` 中 `.answer-reveal` 是否带 `.show`？
- [ ] 试卷有几份就要检查几份（`grep` 搜索 `@media print` 所在文件批量排查）
- [ ] `score-box` 在打印中需要显示（`display:block!important`），这个保留

## 新沉淀：index.html 导航更新规范

项目根目录 `index.html` 是试卷导航总入口，新增试卷时必须同步更新。

### 更新项

1. **卡片徽标**（`<span class="badge">`）：+1
2. **年级标签计数**（`<span class="count">`）：+1
3. **底部总计数**（`共 X 份试卷`）：+1
4. **插入试卷链接**：在对应科目列表中按日期顺序插入
...
项目中的试卷 HTML 文件全部是**单行 minified**，不能用传统的行定位方式编辑。

### 原则

- **不要用 Write 重写整个文件**（除非文件是你刚创建的、或需要全部替换内容）
- 用 `edit` 工具 + **唯一字符串**定位，通过 grep 先找到文件中独一无二的片段
- 找片段时选 CSS 属性值、JS 变量赋值、或带特殊字符的 HTML 属性

### 在创建时使用 Write（一次性写入）

新创建的试卷用 `write` 一次性写入完整内容。需要确保：

- 所有 18 个 DOM ID 齐全（q1~q12、q7a/q8a/q9a/q10a、ta11/ta12）
- SKEY/FKEY/FALT 与题目一致
- SK（localStorage key）全局唯一
- SVG 坐标与角度标注一致
...
```bash
# 先用 grep 确认要替换的字符串在文件中唯一
grep ".answer-reveal{display:block" *.html
# 确认唯一后，用 edit 替换
